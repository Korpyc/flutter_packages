...
export 'package:scharts_flutter_core/core.dart'
    ...
    if (fillPaint.style == PaintingStyle.fill) {
      ...
      canvas.drawLine(
        ...
        Paint()
          ...
          ..strokeWidth = seriesRendererDetails.series.borderWidth,
      );
    }
...
void calculateImage(int? index, CartesianStateProperties stateProperties,
    [SeriesRendererDetails? seriesRendererDetails,
  ...
  if (chart != null && seriesRendererDetails == null) {
    if (chart.plotAreaBackgroundImage != null) {
      ...
      (trackballBehavior.markerSettings!.image != null ||
          seriesRendererDetails!.series.markerSettings.image != null)) {
    if (index != null) {
      if (stateProperties.trackballMarkerSettingsRenderer.images.isNotEmpty &&
          index <=
              stateProperties.trackballMarkerSettingsRenderer.images.length -
                  1) {
        stateProperties.trackballMarkerSettingsRenderer.images.insert(
            index,
            await _getImageInfo(seriesRendererDetails!.series.markerSettings
                .image!)); //trackballBehavior.markerSettings!.image!);
        stateProperties.trackballMarkerSettingsRenderer.images
            .removeAt(index + 1);
        stateProperties.repaintNotifiers['trackball']!.value++;
      } else {
        stateProperties.trackballMarkerSettingsRenderer.images.add(
            await _getImageInfo(seriesRendererDetails!.series.markerSettings
                .image!)); //trackballBehavior.markerSettings!.image!);
        stateProperties.repaintNotifiers['trackball']!.value++;
      }
      ...
      stateProperties.trackballMarkerSettingsRenderer.images.add(
          await _getImageInfo(seriesRendererDetails!
              .series.markerSettings.image!)); //trackballBehavior.markerSett
      stateProperties.repaintNotifiers['trackball']!.value++;
    }
  } else {

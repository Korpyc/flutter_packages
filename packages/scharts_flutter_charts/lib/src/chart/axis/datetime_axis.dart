...
class DateTimeAxisRenderer extends ChartAxisRenderer {
  ...
  void generateVisibleLabels() {
    ...
    int interval;
    if (_axisDetails.seriesRenderers.isNotEmpty &&
        _axisDetails.seriesRenderers.first is StackedColumnSeriesRenderer) {
      final SeriesRendererDetails seriesRendererDetails =
          SeriesHelper.getSeriesRendererDetails(
              _axisDetails.seriesRenderers.first);
      interval = seriesRendererDetails.dataPoints.first.xValue;
    } else
      interval = _axisDetails.visibleRange!.minimum;
    interval = _axisDetails._alignRangeStart(
      this,
      interval,
      _axisDetails.visibleRange!.interval,
    );
    // was added for stacked column series
    if (_axisDetails.seriesRenderers.isNotEmpty &&
        _axisDetails.seriesRenderers.first is StackedColumnSeriesRenderer) {
...
class DateTimeAxisDetails extends ChartAxisRendererDetails {
  ...
  DateTime _increaseDateTimeInterval(
    ...
    if (isIntervalDecimal) {
      ...
      if (axisRenderer._axisDetails.seriesRenderers.isNotEmpty) {
        if (axisRenderer._axisDetails.actualRange?.maximum != null &&
            dateTime.millisecondsSinceEpoch >
                axisRenderer._axisDetails.visibleRange!.maximum) {
          dateTime = DateTime.fromMillisecondsSinceEpoch(
            axisRenderer._axisDetails.visibleRange!.maximum,
          );
          if (axisRenderer._axisDetails.visibleLabels.last.value ==
              dateTime.millisecondsSinceEpoch) {
            dateTime = DateTime.fromMillisecondsSinceEpoch(
                dateTime.millisecondsSinceEpoch + 1);
          }
        }
      }
    } else {

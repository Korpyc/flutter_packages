    ...
    for (; tempInterval <= maximumVisibleRange; tempInterval += interval) {
      ...
      if (minimumVisibleRange <= maximumVisibleRange &&
        ...
        if (numericAxis.numberFormat != null) {
          ...
          if (text.length > 1) {
            String? suffix;
            if (maximumVisibleRange >= 1000 && maximumVisibleRange < 1000000) {
              suffix = 'K';
            } else if (maximumVisibleRange >= 1000000 &&
                maximumVisibleRange < 1000000000) {
              suffix = 'M';
            } else if (maximumVisibleRange >= 1000000000 &&
                maximumVisibleRange < 1000000000000) {
              suffix = 'B';
            } else if (maximumVisibleRange >= 1000000000000 &&
                maximumVisibleRange < 1000000000000000) {
              suffix = 'T';
            } else if (maximumVisibleRange >= 1000000000000000 &&
                maximumVisibleRange < 1000000000000000000) {
              suffix = 'Q';
            }

            if (suffix != null && suffix != text.characters.last.toString()) {
              if (int.tryParse(text.characters.last) == null) {
                text = text.replaceRange(text.length - 1, null, '');
              }
              final double formatedNumber = int.parse(text).toDouble();
              text = (formatedNumber / 1000).toString();
              text += ' $suffix';
            } else {
              if (int.tryParse(text.characters.last) == null) {
                text = text.replaceRange(
                    text.length - 1, null, ' ${text.characters.last}');
              }
            }
          }
        }

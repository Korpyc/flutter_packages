...
import 'package:scharts_flutter_core/core.dart';
import '../../../charts.dart';
...
class ChartAxisRendererDetails {
  ...
  void _drawHorizontalAxesLabels(
    ...
    if (renderType == axis.labelPosition.toString().split('.')[1]) {
      ...
      for (int i = 0; i < visibleLabels.length; i++) {
        ...
        labelRotation = angle;
        final Size textSize = measureText(
            labelText, textStyle, null, label.subtitle, label.subtitleStyle);
        final Size rotatedTextSize = measureText(
            labelText, textStyle, angle, label.subtitle, label.subtitleStyle);
        ...
        if (label._labelCollection != null &&
          ...
          for (int j = 1; j < label._labelCollection!.length; j++) {
            ...
            drawText(
              canvas,
              wrapTxt,
              Offset(pointX, finalTextStartPoint),
              textStyle,
              angle,
              isRtl,
              label.subtitle,
  ...
  void _findNormalRange(
    ...
    if (minimum == 0) {
      interval = (axisRenderer is NumericAxisRenderer)
          ? _calculateCloseRoundInterval(
              end,
            )
          : axisRenderer.calculateInterval(
              VisibleRange(minimum, maximum), axisSize)!;
      maximum = _calculateMaxOptimalValue(
        end,
      ); //(maximum / interval).ceil() * interval;
      /*   interval = (axisRenderer is NumericAxisRenderer)
  ...
  num _calculateCloseRoundInterval(num maxValue) {
    if (maxValue <= 5) {
      return 1;
    }
    return _calculateMaxOptimalValue(maxValue) / 5;
  }

  num _calculateMaxOptimalValue(num maxValue) {
    if (maxValue <= 5) {
      return 5;
    }
    int countPer100 = 1;
    int countPer10 = 1;
    double smallestValue = maxValue.ceil().toDouble();
    while (smallestValue > 1000) {
      smallestValue /= 1000;
      countPer100++;
    }
    ...
    double maxRoundValue = tval;
    if (countPer10 > 1) {
      maxRoundValue *= 10;
    }
    if (countPer100 > 1)
      for (int i = countPer100; i > 1; i--) {
        maxRoundValue = maxRoundValue * 1000;
      }
  ...
  void triggerLabelRenderEvent(
    ...
    if (axis.axisLabelFormatter != null) {
      ...
      subtitle = axisLabel.subtitle;
      subtitleStyle = axisLabel.subtitleStyle;
    }
    ...
    if (visibleLabels.isNotEmpty &&
        visibleLabels.last.text == (eventActualText ?? actualText)) {
      // was added for stacked areas
      if (seriesRenderers.first is StackedAreaSeriesRenderer)

...
import 'package:scharts_flutter_core/localizations.dart';
import 'package:scharts_flutter_core/theme.dart';
import 'package:scharts_flutter_core/tooltip_internal.dart';
...
class SfCartesianChartState extends State<SfCartesianChart>
  ...
  Widget _renderChartElements(BuildContext context) {
    if (widget.plotAreaBackgroundImage != null || widget.legend.image != null) {
      calculateImage(null, _stateProperties);
    }
    _stateProperties.renderingDetails.deviceOrientation =
        MediaQuery.of(context).orientation;
    return Expanded(
                  ...
                  child: GestureDetector(
                      /*  onTapDown: (TapDownDetails details) {
                        if (_stateProperties.chartState.mounted) {
                          ...
                          if (selectionSeriesDetails.seriesType ==
                              'stackedcolumn') {
                            if (selectionSeriesDetails.visible ?? false) {
                              ...
                              if (selectionSeriesDetails.isSelectionEnable ==
                                      true &&
                                  selectionDetails?.selectionRenderer != null &&
                                  selectionSeriesDetails.isOuterRegion ==
                                      false) {
                          ...
                          if (selectionSeriesDetails.visible ?? false) {
                            ...
                            if (selectionSeriesDetails.isSelectionEnable ==
                              ...
                              selectionDetails?.selectionRenderer
                                      ?.seriesRendererDetails =
                                  selectionSeriesDetails;
                              selectionBehaviorRenderer.onTouchEnd();
                            }
                          }
                        }
                      }, */
                      onPanStart: (DragStartDetails details) {
                        ...
                        if (selectionSeriesDetails.seriesType ==
                            'stackedcolumn') {
                          final List<CartesianChartPoint<dynamic>>?
                              nearestDataPoints = getNearestChartPoints(
                                  position.dx,
                                  position.dy,
                                  selectionSeriesDetails
                          ...
                          if (selectionSeriesDetails.visible ?? false) {
                            ...
                            if (selectionSeriesDetails.isSelectionEnable ==
                              ...
                              if (nearestDataPoints != null &&
                                  nearestDataPoints.isNotEmpty) {
                                final ChartLocation xPos = calculatePoint(
                                  nearestDataPoints.first.xValue,
                                  nearestDataPoints.first.yValue,
                                  selectionSeriesDetails.xAxisDetails!,
                                  selectionSeriesDetails.yAxisDetails!,
                                  false,
                                  selectionSeriesDetails.series,
                                  calculatePlotOffset(
                                      selectionSeriesDetails.stateProperties
                                          .chartAxis.axisClipRect,
                                      Offset(
                                          selectionSeriesDetails
                                              .xAxisDetails!.axis.plotOffset,
                                          selectionSeriesDetails
...
/*   void _bindPlotBandWidgets(bool shouldRenderAboveSeries) {
  ...
  void _bindSeriesWidgets() {
    ...
    if (_stateProperties.chartSeries.visibleSeriesRenderers.isNotEmpty) {
      // this code needs to find seriaes type & depend on it select series rendering order
      final XyDataSeriesRenderer firstSeriesRender = _stateProperties
          .chartSeries.visibleSeriesRenderers[0] as XyDataSeriesRenderer;
      final SeriesRendererDetails firstSeriesRenderDetails =
          SeriesHelper.getSeriesRendererDetails(firstSeriesRender);

      if (firstSeriesRenderDetails.seriesType == 'stackedcolumn' ||
          firstSeriesRenderDetails.seriesType == 'stackedarea') {
        for (int i =
  ...
  Widget _bindAxisWidgets(String renderType) {
    // ignore: unnecessary_null_comparison
  ...
  void _performPanDown(DragDownDetails details) {
    ...
    final SeriesRendererDetails selectionSeriesDetails =
        SeriesHelper.getSeriesRendererDetails(selectionseriesRenderer);
    if (selectionSeriesDetails.seriesType == 'stackedcolumn') {
      if (selectionSeriesDetails.visible ?? false) {
        final SelectionBehaviorRenderer? selectionBehaviorRenderer =
            selectionSeriesDetails.selectionBehaviorRenderer;
        final SelectionDetails? selectionDetails =
        ...
        if (selectionSeriesDetails.visible ?? false) {
          ...
          if (selectionSeriesDetails.isSelectionEnable == true &&
            ...
            selectionDetails?.selectionRenderer?.seriesRendererDetails =
                selectionSeriesDetails;
            selectionBehaviorRenderer.onTouchEnd(
                /*  _touchPosition!.dx, _touchPosition!.dy */);
          }
        }
      }
